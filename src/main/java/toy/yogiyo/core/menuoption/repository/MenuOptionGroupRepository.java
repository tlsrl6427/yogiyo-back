...
public interface MenuOptionGroupRepository extends JpaRepository<MenuOptionGroup, Long> {
    ...
    @Query("select mog from MenuOptionGroup mog join fetch mog.menuOptions where mog.id = :menuOptionGroupId")
    Optional<MenuOptionGroup> findWithMenuOptionById(@Param("menuOptionGroupId") Long menuOptionGroupId);
}
